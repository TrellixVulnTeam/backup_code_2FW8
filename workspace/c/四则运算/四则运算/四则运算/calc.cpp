...
class Constant :public Expression
{
    ...
    friend void treeDLR(Expression* o);
    friend void treeLDR(Expression* o);
    friend void treeLRD(Expression* o);
public:
...
void treeDLR(Expression* o) {
    if (o->leftChild == nullptr && o->rightChild == nullptr) {
        cout << ((Constant*)o)->value << " ";
    }
    else {
        cout << ((Operation*)o)->type << " ";
    }
    if (o->leftChild) {
        treeDLR(o->leftChild);
    }
    ...
    if (o->rightChild) {
        treeDLR(o->rightChild);
    }
}

void treeDLRRun(Expression* o) {
    cout << "前序遍历：";
    treeDLR(o);
    cout << endl;
...
void treeLDR(Expression* o) {
    ...
    if (o->rightChild) {
        treeLDR(o->rightChild);
    }
}

void treeLDRRun(Expression* o) {
    cout << "中序遍历：";
    treeLDR(o);
    cout << endl;
...
void treeLRD(Expression* o) {
    if (o->leftChild) {
        treeLRD(o->leftChild);
    }

    if (o->rightChild) {
        treeLRD(o->rightChild);
    }
...
bool isExpression(string& es) {
    string all = "+-*/()1234567890";
        size_t ia = 0;
    for (ia = 0; ia < es.size(); ia++) {
        if (all.find(es[ia]) == -1) {
            return false;
        }
    }
