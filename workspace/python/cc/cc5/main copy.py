...
def split_text_into_tokens(text):
    pattern_word = re.compile(r'\b[A-Z][a-z]+\'?[a-z]+\b', re.S)
    return pattern_word.findall(text)


def split_text_into_tokens2(text):
    pattern_word = re.compile(r'\b[A-Z][a-z]+\'?[a-z]+\b\s\b[A-Z][a-z]+\'?[a-z]+\b', re.S)
    return pattern_word.findall(text)
...
def bi_grams(tokens):
    return [set(token.split(" ")) for token in tokens]
...
def find_characters_v2(text, stoplist, top):
    ...
    words = split_text_into_tokens2(text)
    words = bi_grams(words)
    print(words)

    words_set = filter(filter_by_word, set(words))
    
    word_counts = [(word, words.count(word)) for word in words_set]
...
answer2 = find_characters_v2(text, stop_words, 15)
